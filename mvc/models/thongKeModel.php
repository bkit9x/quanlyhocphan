...
class thongKeModel extends DB
{
    public function tongquat()
    {
        ...
        $query_chuahoc = $this->query("SELECT COUNT(*) as `count`, SUM(`hocphan`.`sotinchi`) as `sum` FROM `hocphan`, `caytientrinh_hocphan`, `caytientrinh_sinhvien` WHERE `caytientrinh_hocphan`.`mahocphan` = `hocphan`.`mahocphan` AND `caytientrinh_hocphan`.`idcaytientrinh` = `caytientrinh_sinhvien`.`idcaytientrinh` AND `caytientrinh_sinhvien`.`idsinhvien`='$sinhvien' AND `hocphan`.`mahocphan` NOT IN(SELECT `diem`.`idhocphan` FROM `diem` WHERE `diem`.`idsinhvien`='$sinhvien');");
        ...
        $query_chuadat = $this->query("SELECT COUNT(*) as `count`, SUM(`hocphan`.`sotinchi`) as `sum` FROM `hocphan`, `diem` WHERE `diem`.`idhocphan` = `hocphan`.`mahocphan` AND `diem`.`diem` < 4 AND `diem`.`idsinhvien`='$sinhvien';");
    ...
    public function chuahoc()
    {
        ...
        $sinhvien = addslashes($_GET['sv']);
        $arr = array();
        $result = $this->query("SELECT `hocphan`.`mahocphan`, `hocphan`.`tenhocphan`, `hocphan`.`sotinchi`, `hocphan`.`hocphantienquyet` as `mahocphantienquyet` FROM `hocphan`, `caytientrinh_hocphan`, `caytientrinh_sinhvien` WHERE `caytientrinh_hocphan`.`mahocphan` = `hocphan`.`mahocphan` AND `caytientrinh_hocphan`.`idcaytientrinh` = `caytientrinh_sinhvien`.`idcaytientrinh` AND `caytientrinh_sinhvien`.`idsinhvien`='$sinhvien' AND `hocphan`.`mahocphan` NOT IN(SELECT `diem`.`idhocphan` FROM `diem` WHERE `diem`.`idsinhvien`='$sinhvien');");
    ...
    public function chuadat()
    {
        ...
        $sinhvien = addslashes($_GET['sv']);
        $arr = array();
        $result = $this->query("SELECT `hocphan`.`mahocphan`, `hocphan`.`tenhocphan`, `hocphan`.`sotinchi`, `diem`.`diem` FROM `hocphan`, `diem` WHERE `diem`.`idhocphan` = `hocphan`.`mahocphan` AND `diem`.`diem` < 4 AND `diem`.`idsinhvien`='$sinhvien';");
        if ($result && $result->num_rows > 0)
            while ($row = $result->fetch_assoc())
                $arr[] = $row;
        return $arr;
