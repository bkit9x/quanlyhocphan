<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Lỗi Trang</title>
    <?php require_once('pages/style.php'); ?>
    <style>
    body {
        background-image: url(data:image/png;base64,);}
        .error-template {padding: 40px 15px;text-align: center;}
        .error-actions {margin-top:15px;margin-bottom:15px;}
        .error-actions .btn { margin-right:10px; }
        </style>
</head>
<body>
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <div class="error-template">
                <h1>
                    Oops!</h1>
                <h2>
                    Có lỗi xảy ra</h2>
                <div class="error-details">
                    Trang bạn yêu cầu không tồn tại, hoặc bạn không có quyền truy cập!
                </div>
                <div class="error-actions">
                    <?php if (isset($_SERVER['HTTP_REFERER'])) {
    echo '<a href="'.$_SERVER['HTTP_REFERER'].'" class="btn btn-primary btn-lg"><span class="glyphicon glyphicon-trangchu"></span>
                            Quay trở trang trước</a><a href="" class="btn btn-default btn-lg"><span class="glyphicon glyphicon-envelope"></span> Về trang chủ </a>';
} else {
    echo '<a href="'.$config->domain.'/trangchu" class="btn btn-primary btn-lg"><span class="glyphicon glyphicon-trangchu"></span>
                            Về trang chủ</a>';
}
                    ?>
                </div>
            </div>
        </div>
    </div>
</div>
</body>
</html>